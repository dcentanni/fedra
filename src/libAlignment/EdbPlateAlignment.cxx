...
void EdbPlateAlignment::Align(EdbPattern &p1, EdbPattern &p2, float dz, int flag)
{
  ...
  Log(1, "EdbPlateAlignment::Align", "pattern 1 with %d in limits x:(%12.2f %12.2f) y:(%12.2f %12.2f)",
      p1.N(), p1.Xmin(), p1.Xmax(), p1.Ymin(), p1.Ymax());
  Log(1, "EdbPlateAlignment::Align", "pattern 2 with %d in limits x:(%12.2f %12.2f) y:(%12.2f %12.2f)",
...
END:
  ...
  ProduceReport();
  if (flag)
    AssignFlagToS(flag); // assign particular flag to the selected segments
  if (eSaveCouples)
    SaveCouplesTree();
}
...
void EdbPlateAlignment::ProduceReport()
{
  ...
  if (eDoCoarse && eCoarseOK)
  {
    ...
    if (npk < eCoarseMin)
      eStatus = false; // too few coinsidences
    else if (npk < 3 * Sqrt(eH_xy_coarse.eMean[0]))
      eStatus = false; // too small signal/noise
    else
      eStatus = true; // good alignment
  ...
  if (eOutputFile)
  {
    ...
    c->Divide(3, 3);
    c->Draw();
    TH2F *hp1 = ePC[0].DrawH2("hp1", "Pattern1");
    c->cd(1);
    hp1->SetStats(0);
    hp1->Draw("colz");
    TH2F *hp2 = ePC[1].DrawH2("hp2", "Pattern2");
    c->cd(2);
    hp2->SetStats(0);
    hp2->Draw("colz");
    if (zphiCoarse)
    {
      c->cd(3);
      zphiCoarse->SetStats(0);
      zphiCoarse->Draw("colz");
    }
    if (xyCoarse)
    {
      c->cd(4);
    ...
    if (n < 1000)
    {
      TH2F *h2 = new TH2F("plateXY", "plateXY", 100, xmin - 10, xmax + 10, 100, ymin - 10, ymax + 10);
      h2->SetStats(0);
      h2->Draw();
      for (int i = 0; i < n; i++)
      {
    ...
    {
      TProfile2D *h3 = new TProfile2D("plateXY", "plateXY", 100, xmin - 10, xmax + 10, 100, ymin - 10, ymax + 10, 0, 10);
      h3->SetStats(0);
      for (int i = 0; i < n; i++)
      {
        EdbSegP *s1 = (EdbSegP *)eS[0].UncheckedAt(i);
        ...
        float x1 = X(0, *s1), y1 = Y(0, *s1);
        float x2 = X(1, *s2), y2 = Y(1, *s2);
        h3->Fill(x1, y1, Sqrt((x2 - x1) * (x2 - x1)) + (y2 - y1) * (y2 - y1));
      }
      h3->Draw("colz");
    }
    ...
    {
      TProfile2D *h3t = new TProfile2D("plateTXTY", "plateTXTY", 100, xmin - 10, xmax + 10, 100, ymin - 10, ymax + 10, 0, 0.02);
      h3t->SetStats(0);
      for (int i = 0; i < n; i++)
      {
        EdbSegP *s1 = (EdbSegP *)eS[0].UncheckedAt(i);
        ...
        float tx1 = TX(0, *s1), ty1 = TY(0, *s1);
        float tx2 = TX(1, *s2), ty2 = TY(1, *s2);
        h3t->Fill(x1, y1, Sqrt((tx2 - tx1) * (tx2 - tx1)) + (ty2 - ty1) * (ty2 - ty1));
      }
      h3t->Draw("colz");
    }
    ...
    for (int i = 1; i < nbin + 1; i++)
      xbins[i] = Sqrt(xbins[i - 1] * xbins[i - 1] + s0);
    ...
    TText *t = new TText();
    t->SetTextSize(0.015);
    t->DrawText(0.25, 0.0001, Form("%s/%s    %s", gSystem->WorkingDirectory(), eOutputFile->GetName(), time.AsString()));
...
void EdbPlateAlignment::FineAlAff(EdbPattern &p1, EdbPattern &p2, EdbLayer &la1)
{
  ...
  EdbAffine2D aff;
  if (eNoScale)
    CalculateAffXYTurn(sel1, sel2, aff);
  else if (eNoScaleRot)
    CalculateAffXYShift(sel1, sel2, aff);
  else
...
void EdbPlateAlignment::SlowAlignXY(EdbPattern &pat1, EdbPattern &pat2, EdbH2 &hxy, EdbH1 &hphi, const char *name)
{
  ...
  if (!f || f->IsZombie())
    Log(1, "EdbPlateAlignment::SlowAlignXY", "ERROR! can not open file %s", name);
