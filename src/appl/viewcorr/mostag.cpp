...
int main(int argc, char *argv[])
{
  ...
  else if (do_file)
  {
    ...
    from_fragment = 0;
    n_fragments = 1;
    TagSide(id, pat0, from_fragment, n_fragments, 0, cenv, mio, omio);
    omio.SaveFragmentObj(&pat0, id.ePlate, 0, 0, "gpat");
    Log(1, "mostag", "save %d to %s", pat0.N(), omio.GetFileName());
  }
...
void DrawOut(const EdbPattern &p, EdbMosaicIO &omio)
{
  ...
  gStyle->SetOptStat("n");
  pdb->cd(2);
  gOH.dbltxty->Draw("colz");
  gStyle->SetOptStat("n");

  if (do_save_canvas)
    omio.SaveFragmentObj(c, p.Plate(), p.Side(), p.ID(), "peaks");
  if (do_save_gif)
    c->Print(omio.FileName(p.Brick(), p.Plate(), p.Side(), p.ID(), "", ".gif").c_str());
  delete c;
}
...
void TagSide(EdbID id, EdbPattern &pat, int from, int nfrag, int side, TEnv &cenv, EdbMosaicIO &mio, EdbMosaicIO &omio)
{
  ...
  if (str)
    sscanf(str, "%f %f", &thetaMin, &thetaMax);
...
void FindPeaks(EdbH2 &h2p, EdbPattern &ptag, int npmax, float minVol)
{
  ...
  for (int i = 0; i < npmax; i++)
  {
    ...
    if (pf.ePeak[i] > minVol)
      ptag.AddSegment(i, pf.eXpeak[i], pf.eYpeak[i], 0, 0, pf.ePeak[i]);
  }
